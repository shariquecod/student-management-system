...
export function ClassesStatSkeleton() {
  return (
    <>
...
export function ClassesTableSkeleton({ rows = 8 }: { rows?: number }) {
  return (
    <>
      {Array.from({ length: rows }).map((_, i) => (
        <tr key={i} className="students-table-row">
          ...
          <td className="hidden md:table-cell">
            <div className="dashboard-skeleton h-5 w-8 rounded" />
          </td>
          <td>
            <div className="dashboard-skeleton h-5 w-10 rounded-full" />
          </td>
          <td className="hidden lg:table-cell">
            <div className="dashboard-skeleton h-2.5 w-28 rounded" />
          </td>
...
export function ClassesCompactSkeleton({ rows = 8 }: { rows?: number }) {
  return (
    <>
      {Array.from({ length: rows }).map((_, i) => (
        <div
